...
def draw_plots( data_file_path, main_particle_mass, range_minimum, range_maximum ): 
    ...
    counts, bins, patches = plt.hist(data_list, facecolor='yellow', edgecolor='gray', range=(range_minimum, floatrange_minimum)) 
    ...
    main_particle_proportion = (main_particle_mass / range_maximum)*100 # The proportion of mass of the main particle being searched 
    other_particle_proportion = ((range_maximum - main_particle_mass) / range_maximum )*100# The proportion of mass of the other particles present in collision 

    print "Main particle proportion %d"%( main_particle_proportion )
    print "Other particle proportion %d"%( other_particle_proportion )
    print "Minimum range value %d"%( range_minimum )
    print "Maximum range value %d"%( range_maximum )
	
    # Change the colors of bars at the edges...
    main_particles, other_particles = np.percentile( data_list, [ main_particle_proportion, other_particle_proportion ] )
...
if __name__ == "__main__": 
    ...
    draw_plots (args.file[0], args.particle_mass[0], float(args.range[0]), float(args.range[1]))
    # draw_plots (args.file[0], float(args.particle_mass[0]), args.range)
